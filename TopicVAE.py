...
ARG.add_argument('--beta', type=float, default=0.2, help='Strength of feature-level disentanglement, in (0,oo).')
ARG.add_argument('--tau', type=float, default=0.1, help='Temperature of sigmoid/softmax, in (0,oo).')
ARG.add_argument('--std', type=float, default=0.3, help='Standard deviation of the Gaussian prior.')
...
if ARG.mode == 'tst':
    # Musical-topicvae-4-13-22-26-22, Industrial-topicvae-4-13-20-53-12, Beauty-topicvae-4-13-22-26-38
    LOG_DIR = 'checkpoints/Industrial-topicvae-4-13-20-53-12'

def set_rng_seed(seed):
    np.random.seed(seed)
    tf.set_random_seed(seed)

class TopicVAE(object):
    ...
    def build_graph(self):
        saver, logits, recon_loss, kl, info_nec_loss = self.forward_pass()
        ...
        neg_elbo = recon_loss + self.anneal_ph * kl + self.lam * info_nec_loss + 2. * reg_var
    ...
    def q_graph_k(self, x):
        ...
        for i, (w, b) in enumerate(zip(self.weights_q, self.biases_q)):
            ...
            if i != len(self.weights_q) - 1:
                ...
                a = h[:, :self.q_dims[-1]]
                mu_q = tf.nn.l2_normalize(a, axis=1)
                b = h[:, self.q_dims[-1]:]
                std_q = tf.exp(-0.5 * b) * ARG.std
                # Trick: KL is constant w.r.t. to mu_q after we normalize mu_q.
                kl = tf.reduce_mean(tf.reduce_sum(0.5 * (b + tf.exp(-b) - 1.), axis=1))
    ...
    def forward_pass(self):
        ...
        for t in range(ARG.tfac):
            ...
            mu_t, std_t, kl_t = self.q_graph_k(x_t)
            epsilon = tf.random_normal(tf.shape(std_t))
            z_t = mu_t + self.is_training_ph * epsilon * std_t
            kl = (kl_t if (kl is None) else (kl + kl_t))
        ...
        # info_nec_loss = 0.0
        return tf.train.Saver(), logits, recon_loss, kl, info_nec_loss
        ...
        with tf.Session(config=config) as sess:
            ...
            for epoch in range(ARG.epoch):
                ...
                for bnum, st_idx in enumerate(range(0, n, ARG.batch)):
                    ...
                    x = x.astype('float32')
                    if epoch > 100:
                        base = ((epoch - 100) % 100) * 0.05
                        step = (update_count % num_batches) * 0.01
                        anneal = base + step
                    else:
                        anneal = max((update_count / num_batches) * 0.01, ARG.beta)
...
def main():
    ...
    if ARG.mode in ('trn',):
        ...
        model = TopicVAE(n_items, item_words, word_embs)
